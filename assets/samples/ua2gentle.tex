% This file is an adaptation of Michael Doob's Gentle Introduction
% to TeX. Most of the content is identical, but where `gentle.tex` 
% is meant to be compiled with the plain fmt, this file is meant to
% be compiled with the ua2plain fmt. 
...
\font\tenrm=STIXTwoText-Regular.otf at 10pt
\font\tenit=STIXTwoText-Italic.otf at 10pt
\font\tenbf=STIXTwoText-Bold.otf at 10pt
\font\tentt=JetBrainsMono-Regular.otf at 8.5pt

\font\brm=STIXTwoText-Regular.otf at 12pt
\font\halfrm=STIXTwoText-Regular.otf at 11pt
\font\bbrm=STIXTwoText-Regular.otf at 14.4pt
\font\bbbrm=STIXTwoText-Regular.otf at 17.28pt
\font\bbbbrm=STIXTwoText-Regular.otf at 20.74pt
\font\bbbbbrm=STIXTwoText-Regular.otf at 24.88pt

\font\sf=FiraSans-Regular.otf at 10pt
\font\chapfont=STIXTwoText-Bold.otf at 14.4pt
\font\secfont=STIXTwoText-Bold.otf at 12pt

% Mode-safe symbols
\Umathchardef\bullet=2 0 "02219
\def\dag{\ifmmode\dagger\else$\dagger$\fi}
\def\ddag{\ifmmode\ddagger\else$\ddagger$\fi}
\def\S{\ifmmode\mathsection\else$\mathsection$\fi}
\def\P{\ifmmode\mathparagraph\else$\mathparagraph$\fi}
\def\to{\rightarrow}
\def\gets{\leftarrow}
\def\infty{\Umathchar 0 0 "0221E }
\def\ge{\geq}
\def\le{\leq}

\def\star{\Umathchar 2 0 "02216 }
\def\circ{\Umathchar 2 0 "02218 }
\def\div{\Umathchar 2 0 "000F7 }
\def\diamond{\Umathchar 2 0 "022C4 }
\def\cap{\Umathchar 2 0 "02229 }
\def\cup{\Umathchar 2 0 "0222A }
\def\varepsilon{\Umathchar 7 0 "003B5 }
\def\vartheta{\Umathchar 7 0 "003D1 }
\def\varrho{\Umathchar 7 0 "003F1 }
\def\varphi{\Umathchar 7 0 "003D5 }
\def\chi{\Umathchar 7 0 "003C7 }

% Spelling and logic
\newif\ifcanspell \canspelltrue
\let\center=\centerline
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\def\shouldbefmt{ua2plain}
\ifx\shouldbefmt\fmtname
\else
\immediate\write16{}
\immediate\write16{***** WARNING WARNING WARNING *****}
\immediate\write16{You probably need to use the command}
\immediate\write16{nematex --fmt=ua2plain \string<filename\string>}
\immediate\write16{to make your pdf file.}
\immediate\write16{***** WARNING WARNING WARNING *****}
\immediate\write16{}
\fi
...
\def\section#1{\par
            ...
            \ifwritingcontents \write\contents{\string\line\string{#1 
                   ...
                   \string}}\fi 
            \titlepagetrue} 
 \def\subsection#1{\par \advance\subsecnum by 1 
                \vskip 30pt 
                \tagstructbegin{H2}%
...
\def\exercise{\global\advance \exno by 1 
         \vskip\baselineskip 
         \noindent \tagstructbegin{P}$\triangleright$ Exercise \the\secnum.\the\exno\tagstructend\quad 
         } 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\def\endliteral{\endgroup\tagstructend} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\def\beginuser{\vskip\parskip 
               ...
               \tagstructbegin{Code}%
               \begingroup 
               \tt \obeyspaces \obeylines \parskip=0pt \parindent=0pt} 
 
\def\enduser{\endgroup\tagstructend} 
...
\def\writetime{%
      \ifnum \hour=0 12\else
      \ifnum \hour<13 \number\hour\else
      \advance \hour by -12 \number\hour \advance \hour by 12 \fi\fi:%
...
Lincoln Durst (Providence, RI), Victor Eijkhout (University of Nijmegen),
...
and Dominik Wujastyk (Wellcome Institute for the History of Medicine). 
 \endgroup 
...
\line{\startlink attr {/Border [0 0 0]} goto name {getting_started}1.~Getting Started \dotfill{} 1\endlink} 
...
\line{\qquad{}2.3 Dots, dashes, quotes, \dots \dotfill{} 13} 
...
\line{\startlink attr {/Border [0 0 0]} goto name {math_anxiety}5.~No math anxiety here! \dotfill{} 33\endlink} 
...
\pdfdest name {getting_started} xyz
...
\def\bdots{\dots} %%% boldface version of \ldots 
...
The footline has contained either a centered{} number or, if {\tt 
...
line on the left margin, on the right margin, or centered.  Thus 
\toindex{hfill} 
\toindex{break} 
\toindex{centerline} 
\toindex{leftline} 
...
{\tt \\line\lb left text \\hfil center{} text \\hfil right text.\rb} 
will produce 
\medskip 
\line{left text \hfil center{} text \hfil right text.} 
\toindex{hfil} 
} 

\exercise Typeset the following line: 

\line{left end \hfil left tackle \hfil left guard \hfil center{} \hfil 
right guard \hfil right tackle \hfil right end} 

\exercise Typeset the following line with twice as much space between 
``left'' and ``right-center{}'' as between ``right-center{}'' and 
``right'': 

\line{left \hfil \hfil right-center{} \hfil right} 
...
\exercise Change the footline so that the page number is centered{} 
...
{\tt \\centerline\lb\\bf A bold title\rb} will produce a centered{} 
...
lines of text.  It can also be {\sl displayed}, that is, centered{} 
...
symbols on most terminal keyboards).  To negate a relation, it is 
common to prepend an {\tt n} to the relation's control word (for 
example, {\tt \\nleq} for $\nleq$).  Here 
are some relations: 
\TeXref{436} 
...
\exercise 
Typeset: $\vec x\cdot \vec y \ne 0$ if and only if $\vec x$ is not perpendicular to $\vec y$. 
...
The matrix entries in our examples have all been centered{} 
...
but the last one it is possible to center{} the entry or to 
...
it to be centered{}, just as with the {\tt \\line} control word. 
...
\noindent 
will produce a table with the first column centered{}, the second 
...
\exercise Take the table of Canadian provinces above and center{} each 
...
\exercise One way to center{} a block of text, possibly several 
lines long, is to use: {\tt \$\$\\vbox\lb$\ldots$\rb\$\$}. Use 
this to center{} the table given above.  Does the 
...
example, can be used to display flush left, flush right, or centered. 
...
and also be set as mathematics.  The second section will be centered{} 
...
right rather than being perfectly centered. As in the {\tt \\settabs} 
...
Finally, we can see how to center{} the text by noting that the 
...
that is centered{} on the page, we can do so by putting the {\tt 
...
display.  Hence a centered{} table with this nice spacing may be 
...
{\tt \\address}, {\tt \\city}, {\tt \\state}, and {\tt 
\\zip code}. 
...
between the braces to be put in a paragraph with width 12 centimeter 
s and then centered{} with some space added above and 
...
name of {\tt \\\\centerline}.  This can be done by using the {\tt 
\\let} control word.  The use of {\tt \\let \\centerline = 
...
Many computer center{}s have the \LaTeX{} macro package.  This 
...
\exercise Make the following grid (each box is 1 centimeter{} square): 
\medskip 
...
the box\rb } will produce an hbox exactly five centimeter{}s 
...
Extra space, say one centimeter,  can be added by putting an 
...
\exercise Use the method of boxing material to put text centered{} 
...
\+ \\centerline 26 & \centerline 60 & \\check 36 & \\chi 35 \cr 
